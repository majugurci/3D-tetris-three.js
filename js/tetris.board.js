...
/**
 * Test if moving block has reached the bottom or
 * when rotated is out of boundaries
 * @param ground_check
 * @returns {number}
 */
...
/**
 * Check if any of the slices is complete and if it is
 * remove him and shift all above slices towards bottom
 */
Tetris.Board.checkCompleted = function() {
	...
	if(rebuild) {
			...
			for(var y = 0; y < fields[0].length; y++) {
				for(var x = 0; x < fields.length; x++) {
					...
					if(fields[x][y][z] == Tetris.Board.FIELD.EMPTY && Tetris.staticBlocks[x][y][z]) {
						...
						Tetris.scene.remove(Tetris.staticBlocksEdges[x][y][z]);
        ...
        var number = Tetris.Board.checkHeight();
        Tetris.setPieceHeight(number);
	}
};

/**
 * Test if there is a piece of block on slice
 * Every slice is tested
 * This is used for updating vertical height of blocks next to to game
 * @returns {number}
 */
Tetris.Board.checkHeight = function() {
    var sliceEmpty = true;
    ...
    for (var z = 0; z < fields[0][0].length-1; z++) {
        ...
        for (var y = 0; y < fields[0].length; y++) {
            for (var x = 0; x < fields.length; x++) {
                if (fields[x][y][z] == Tetris.Board.FIELD.PETRIFIED) {
                    sliceEmpty = false;
                }
            }
        }
